...
export default function HomePage() {
  ...
  async function addChallenge(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!title || description.length < 30 || !location) return;
    const category = String(form.get("category") || "Community");
    const response = await fetch("/api/challenges", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, description, category, location }) });
    if (!response.ok) return;
    const result = await response.json();
    const next: Challenge = { id: result.challenge.id, title, description, category, location, status: result.challenge.status, priority: result.challenge.priority, confidence: result.challenge.confidence, submittedBy: "You" };
